...
pkgdatadir = $(datadir)/btscoin
pkgincludedir = $(includedir)/btscoin
pkglibdir = $(libdir)/btscoin
pkglibexecdir = $(libexecdir)/btscoin
...
CONFIG_HEADER = $(top_builddir)/src/config/btscoin-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/btscoin-config.h.in \
...
ACLOCAL = ${SHELL} /home/btscoin-core/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/btscoin-core/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/btscoin-core/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/btscoin-core/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = btscoin-cli
BITCOIN_DAEMON_NAME = btscoind
BITCOIN_GUI_NAME = btscoin-qt
BITCOIN_TX_NAME = btscoin-tx
...
MAINT = 
MAKEINFO = ${SHELL} /home/btscoin-core/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
PACKAGE_NAME = BTSCOIN Core
PACKAGE_STRING = BTSCOIN Core 2.0.0
PACKAGE_TARNAME = btscoin
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /home/btscoin-core
abs_srcdir = /home/btscoin-core
abs_top_builddir = /home/btscoin-core
abs_top_srcdir = /home/btscoin-core
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/btscoin-core/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_btscoin_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_btscoin_coverage.info test_btscoin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/BTSCOIN-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/btscoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/btscoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/btscoin-config.h
$(top_srcdir)/src/config/btscoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/btscoin-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/BTSCOIN-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BTSCOIN-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BTSCOIN-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_btscoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_btscoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_btscoin_filtered.info: test_btscoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_btscoin_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_btscoin_coverage.info: baseline_filtered_combined.info test_btscoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_btscoin_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_btscoin_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_btscoin_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_btscoin.coverage/.dirstamp:  test_btscoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_btscoin.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_btscoin.coverage/ total.coverage/ $(OSX_APP)
